...
<div style="width: 230px; display:flex; flex-wrap: wrap;">
    @for ($i = 1; $i < $hari+1; $i++)
    @if (is_null(\App\Models\absen::where(['user_id' => $user])->where(['tanggal'=>$tahun.'-'.$bulan.'-'.$i])->first()))
...
<hr>
<center>
<button style="margin-bottom: 10px;" class="btn btn-info">Absen!</button></center>
</div>
